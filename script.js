        ...
        */
        ...
        function gerenciarLotacao2() {
            const chkLot1 = document.getElementById('chk-lot1');
            const chkLot2 = document.getElementById('chk-lot2');
            if (chkLot2.checked && !chkLot1.checked) {
                chkLot1.checked = true;
            }
            atualizarFicha();
        }
        function atualizarFicha() {
            ...
            const carreira = get('i-carreira');
            if (carreira) {
                document.getElementById('v-carreira').innerText = carreira;
            } else {
                document.getElementById('v-carreira').innerText = '-----';
            }
            ...
            const isInativo = chkApo || chkDes;
            if (isInativo) {
                document.getElementById('chk-lot1').checked = false;
                document.getElementById('chk-lot2').checked = false;
            }
            ...
            const mostrarLot1 = document.getElementById('chk-lot1').checked;
            ...
            container.innerHTML = '';
            if (isInativo) {
                const semLotacao = document.createElement('div');
                semLotacao.className = 'lotacao-item';
                semLotacao.innerText = '-----';
                container.appendChild(semLotacao);
            } else {
                if (mostrarLot1 && lot1) {
            ...
            const estadosSemConjuge = ['SOLTEIRO', 'VIÚVO', 'SEPARADO JUDICIALMENTE', 'DIVORCIADO'];
            ...
            let msg = '';
            if (arquivoCarregado && !carreira) {
                msg += '* Carreira não preenchida. ';
            }
            if ((estadoCivil === 'CASADO' || estadoCivil === 'UNIÃO ESTÁVEL') && conjugeVazio) {
                msg += '* ' + estadoCivil + ' sem cônjuge';
            ...
            const msgDiv = document.getElementById('estado-civil-msg');
            if (msg.trim() !== '') {
                msgDiv.className = 'aviso-msg';
            } else {
                msgDiv.className = '';
            }
            verificarEAjustarAvisoPiscante();
        }
		...
		function normalizarEstadoCivil(valor) {
			...
			// CORREÇÃO: aceita tanto "SEPARADO" quanto "SEPARACAO"
			if (semPrefixo.includes('SEPARADO') || semPrefixo.includes('SEPARACAO')) return 'SEPARADO JUDICIALMENTE';
        ...
        function processarArquivo(file) {
            ...
            reader.onload = function (e) {
                ...
                const temApo = dados[idx.DATA_APOSENTADORIA] && dados[idx.DATA_APOSENTADORIA] !== '';
                const temDes = dados[idx.DATA_DESLIGAMENTO] && dados[idx.DATA_DESLIGAMENTO] !== '';
                if (temApo || temDes) {
                    document.getElementById('chk-lot1').checked = false;
                    document.getElementById('chk-lot2').checked = false;
                } else {
                    document.getElementById('chk-lot1').checked = true;
                }
        ...
        function resetarFormulario() {
            ...
            document.getElementById('i-cpf').value = '';
            document.getElementById('i-mat').value = '';
            document.getElementById('i-cargo').value = '';
            document.getElementById('i-cod').value = '';
            document.getElementById('i-etapa').value = '';
            document.getElementById('i-carreira').value = '';
            document.getElementById('i-vinculo').value = 'ESTATUTÁRIO';
            document.getElementById('i-ch').value = '';
            document.getElementById('i-adm').value = '';
            document.getElementById('i-apo').value = '';
            document.getElementById('i-des').value = '';
            document.getElementById('i-lot1').value = '';
            document.getElementById('i-lot2').value = '';
            document.getElementById('i-nat').value = '';
            document.getElementById('i-uf-nat').value = '';
            document.getElementById('i-nacionalidade').value = 'BRASILEIRA';
            document.getElementById('i-sexo').value = '';
            document.getElementById('i-nasc').value = '';
            document.getElementById('i-ecivil').value = '';
            document.getElementById('i-mae').value = '';
            document.getElementById('i-pai').value = '';
            document.getElementById('i-conjuge').value = '';
            document.getElementById('i-t-num').value = '';
            document.getElementById('i-t-zona').value = '';
            document.getElementById('i-t-sec').value = '';
            document.getElementById('i-rua').value = '';
            document.getElementById('i-comp').value = '';
            document.getElementById('i-bairro').value = '';
            document.getElementById('i-cid').value = '';
            document.getElementById('i-uf-end').value = '';
            document.getElementById('i-cep').value = '';
            document.getElementById('i-tel').value = '';
            document.getElementById('i-cel1').value = '';
            document.getElementById('i-cel2').value = '';
            ...
            document.getElementById('i-apo').disabled = false;
            document.getElementById('i-des').disabled = false;

            document.getElementById('erro-cpf').innerText = '';
            document.getElementById('erro-mat').innerText = '';
        ...
        document.getElementById('btn-copiar-imagem').addEventListener('click', function () {
            const elemento = document.querySelector('.ficha-grid');
            if (!elemento) {
                mostrarNotificacao('Elemento não encontrado para capturar.');
                return;
            }
            mostrarNotificacao('Gerando imagem...');
            html2canvas(elemento, {
                ...
                canvas.toBlob(function (blob) {
                    try {
                        ...
                        mostrarNotificacao('Imagem copiada para a área de transferência!');
                    } catch (err) {
                        mostrarNotificacao('Não foi possível copiar automaticamente. Clique com botão direito na imagem para salvar.');
                        const win = window.open();
                        win.document.write('<img src="' + canvas.toDataURL() + '" style="max-width:100%;">');
                        win.document.title = 'Captura da Ficha';
                    }
                }, 'image/png');
            }).catch(error => {
                console.error('Erro ao gerar imagem:', error);
                mostrarNotificacao('Erro ao gerar a imagem. Tente novamente.');
            });
        });
        ...
        // Função para verificar se há avisos e mostrar/esconder o modal piscante
        function verificarEAjustarAvisoPiscante() {
            const avisoDiv = document.getElementById('avisoPiscante');
            const msgElement = document.getElementById('estado-civil-msg');
            if (!avisoDiv || !msgElement) return;

            const temAviso = msgElement.innerText.trim() !== '';
            if (temAviso) {
        ...
        // Função para abrir o modal de aviso com a mensagem atual
        function abrirModalAviso() {
            const msgElement = document.getElementById('estado-civil-msg');
            if (!msgElement) return;
            const mensagem = msgElement.innerText.trim();
            if (mensagem === '') return;

            document.getElementById('textoAvisoModal').innerText = mensagem;
            document.getElementById('modalAvisoOverlay').style.display = 'flex';
        }

        function fecharModalAviso() {
            document.getElementById('modalAvisoOverlay').style.display = 'none';
        }

        // Adicionar evento de clique no modal piscante
        document.addEventListener('DOMContentLoaded', function () {
            const avisoPiscante = document.getElementById('avisoPiscante');
            if (avisoPiscante) {
                avisoPiscante.addEventListener('click', abrirModalAviso);
            }
        });
